...
void sendRequest(String email, String password) async {
  final url = Uri.parse('http://localhost:3001/api/authen/login');
  final loginData = {
    'email': email,
    'password': password,
    'role' : 'student'
  ...
  try {
    // Send a POST request with the login data in the body
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'}, // Important for sending JSON
      body: json.encode(loginData), // Encode the data to JSON format
    );
